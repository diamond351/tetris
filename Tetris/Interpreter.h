...
public:
    ...
    bool isIntersect(AbstractBlock *b);     //checks if the new position collides with another block
    bool isOut(AbstractBlock *b);           //checks if the new position is out of the grid
    void update(std::string input);
    void heavyDrop();
    std::string match();
    void execute(std::string command);
    bool isEnd(); 
    void delCells(std::vector<int> rows);   //empties the cells that have been cleared
